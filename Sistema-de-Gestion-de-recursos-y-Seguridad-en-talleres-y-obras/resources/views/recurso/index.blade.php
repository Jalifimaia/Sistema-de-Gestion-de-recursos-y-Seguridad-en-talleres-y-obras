@extends('layouts.app')

@section('template_title')
    Recursos
@endsection

@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">

                            <span id="card_title">
                                {{ __('Recursos') }}
                            </span>

                             <div class="float-right">
                                <a href="{{ route('recursos.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                  {{ __('Create New') }}
                                </a>
                              </div>
                        </div>
                    </div>
                    @if ($message = Session::get('success'))
                        <div class="alert alert-success m-4">
                            <p>{{ $message }}</p>
                        </div>
                    @endif

                    <div class="card-body bg-white">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                <thead class="thead">
                                    <tr>
                                        <th>No</th>
                                        
									<th >Id Categoria</th>
									<th >Id Estado</th>
									<th >Id Incidente Detalle</th>
									<th >Id Usuario Creacion</th>
									<th >Id Usuario Modificacion</th>
									<th >Nombre</th>
									<th >Descripcion</th>
									<th >Costo Unitario</th>
									<th >Fecha Creacion</th>
									<th >Fecha Modificacion</th>

                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($recursos as $recurso)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            
										<td >{{ $recurso->id_categoria }}</td>
										<td >{{ $recurso->id_estado }}</td>
										<td >{{ $recurso->id_incidente_detalle }}</td>
										<td >{{ $recurso->id_usuario_creacion }}</td>
										<td >{{ $recurso->id_usuario_modificacion }}</td>
										<td >{{ $recurso->nombre }}</td>
										<td >{{ $recurso->descripcion }}</td>
										<td >{{ $recurso->costo_unitario }}</td>
										<td >{{ $recurso->fecha_creacion }}</td>
										<td >{{ $recurso->fecha_modificacion }}</td>

                                            <td>
                                                <form action="{{ route('recursos.destroy', $recurso->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('recursos.show', $recurso->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('recursos.edit', $recurso->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm" onclick="event.preventDefault(); confirm('Are you sure to delete?') ? this.closest('form').submit() : false;"><i class="fa fa-fw fa-trash"></i> {{ __('Delete') }}</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                {!! $recursos->withQueryString()->links() !!}
            </div>
        </div>
    </div>
@endsection

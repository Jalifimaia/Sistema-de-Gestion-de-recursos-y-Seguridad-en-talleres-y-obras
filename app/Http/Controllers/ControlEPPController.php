  ...
  public function index()
{
    ...
    // 👉 Directamente paginar los que NO tienen checklist hoy
    $sinChecklist = Usuario::whereHas('rol', function($q){
            $q->where('nombre_rol', 'Trabajador');
        })
        ->whereDoesntHave('checklists', function($q) use ($hoy) {
            $q->whereDate('fecha', $hoy);
        })
        ->with('estado')
        ->paginate(5, ['*'], 'sin_checklist_page');
...
public function sinChecklist()
{
    ...
    return view('controlEPP.index', compact('sinChecklist'));
}
